...
static gboolean
_match_comma_delimiter(const gchar *cur, const gchar **new_cur, gpointer user_data)
{
  *new_cur = cur + 1;
  return (*cur == ',');
}
...
_parse_unquoted_empty_value(ListScanner *self)
{
  ...
  if (_match_comma_delimiter(self->current_arg, &end, NULL))
    {
...
_parse_value_from_current_position(ListScanner *self)
{
  ...
  str_repr_decode_until_delimiter(self->value, self->current_arg, &self->current_arg, _match_comma_delimiter, NULL);
  return TRUE;
}
