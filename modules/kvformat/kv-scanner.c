...
{
  return (self->pair_separator && (*cur == self->pair_separator));
}
...
_match_delimiter(const gchar *cur, const gchar **new_cur, gpointer user_data)
{
  ...
  else if (self->pair_separator)
    {
      result = (*cur == self->pair_separator);
      *new_cur = cur + 1;
    }
  else
    {
...
KVScanner *
kv_scanner_new(gchar value_separator, gchar pair_separator, KVTransformValueFunc transform_value)
{
